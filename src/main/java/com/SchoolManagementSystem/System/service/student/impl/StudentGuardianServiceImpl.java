...
public class StudentGuardianServiceImpl implements StudentGuardianService {
    ...
    public StudentGuardianDto save(StudentGuardianDto dto) {
        ...
        StudentGuardian relation =
                StudentGuardianMapper.toEntity(dto);

        return StudentGuardianMapper.toDto(
    ...
    @Override
    @Transactional
    public StudentGuardianDto connectStudentToGuardian(
            Long studentId,
            Long guardianId,
            Boolean primaryGuardian) {


        if (studentGuardianRepository
                .existsByStudentIdAndGuardianId(
                        studentId,
                        guardianId)) {
        ...
        relation.setPrimaryGuardian(
                Boolean.TRUE.equals(primaryGuardian)
        );
    ...
    public StudentGuardianDto changePrimaryGuardian(Long studentId, Long newGuardianId) {
        ...
        StudentGuardian newGuardianRelation =
                studentGuardianRepository.findByStudentIdAndGuardianId(studentId, newGuardianId)
                        .orElseThrow(() ->
                                new NotFoundException(ErrorCode.RELATION_NOT_FOUND));
        ...
        return StudentGuardianMapper.toDto(
                studentGuardianRepository.save(newGuardianRelation)
        );
    }
    ...
    private void removeCurrentPrimaryGuardian(Long studentId) {
        ...
        studentGuardianRepository
                ...
                .stream()
                .filter(StudentGuardian::getPrimaryGuardian)
                .forEach(relation -> {
    ...
    public List<StudentDto> getStudentsWithoutGuardian() {
        ...
        List<Student> students =
                studentRepository.findStudentsWithoutGuardian();


        if (students.isEmpty()) {

            throw new NotFoundException(
                    ErrorCode.NO_STUDENTS_WITHOUT_GUARDIAN
            );
        }
    ...
    public List<GuardianDto> getGuardiansWithoutStudents() {
        ...
        List<Guardian> guardians =
                guardianRepository.findGuardiansWithoutStudents();
    ...
    private Student findStudent(Long id) {
        return studentRepository.findById(id)
                .orElseThrow(() ->
                        new NotFoundException(ErrorCode.STUDENT_NOT_FOUND));
    }
    ...
    private Guardian findGuardian(Long id) {
        ...
        return guardianRepository.findById(id)
                .orElseThrow(() ->
                        new NotFoundException(ErrorCode.GUARDIAN_NOT_FOUND));
    }
    ...
    public void removeGuardian(Long studentId, Long guardianId) {
        ...
        StudentGuardian studentGuardian = studentGuardianRepository.findByStudentIdAndGuardianId(studentId, guardianId)
                .orElseThrow(() -> new NotFoundException(ErrorCode.RELATION_NOT_FOUND));

        if (studentGuardian.getPrimaryGuardian())
